  ...
  <form method="post">
    <div class="row px-xl-5">
        ...
        <div class="bg-light p-30 mb-5">
          <div class="row">
            <div class="col-md-6 form-group">
              ...
              <input value="${requestScope.user.getFullName()}" name="fullName" class="form-control" type="text" placeholder="Doe John" >
            </div>
            <div class="col-md-6 form-group">
              <label>Date Of Birth</label>
              <input value="${requestScope.user.getBirthDay()}" name="birthDay" class="form-control" type="date" >
            </div>
            <div class="col-md-6 form-group">
              <label>Phone Number</label>
              <input value="${requestScope.user.getPhoneNumber()}" name="phoneNumber" class="form-control" type="text" placeholder="0999888999" >
            </div>
            <div class="col-md-6 form-group">
              <label>Email</label>
              <input value="${requestScope.user.getEmail()}" name="email" class="form-control" type="email" placeholder="nhathoang@gmail.com" >
            </div>
            <div class="col-md-6 form-group">
              <label>Address</label>
              <input value="${requestScope.user.getAddress()}" name="address" class="form-control" type="text" placeholder="123 Street" >
            </div>
            <div class="col-md-6 form-group">
              <label>Image</label>
              <input id="image" onchange="chooseFile()"  value="${requestScope.user.getImage()}" name="image" class="form-control" type="text"  >
            </div>
            <div class="col-md-6 form-group">
              <label>Password</label>
              <input  value="${requestScope.user.getPassword()}" name="password" class="form-control" type="text" >
            </div>
            <div class="col-md-6 form-group">
              <label>Role</label>
              <c:forEach items="${requestScope.listRole}" var="role">
                <c:if test="${requestScope.user.getRole() == role.getId()}">
                  <input value="${role.getId()}" class="form-control" type="number" name="role" min="1" max="2">
                </c:if>
              </c:forEach>
            </div>
            <div class="col-md-6 form-group">
              <label>Create</label>
              <input value="${requestScope.user.getCreatedTime()}" class="form-control" disabled>
            </div>
            <div class="col-md-6 form-group">
              <label>Updated</label>
              <input value="${requestScope.user.getUpdatedTime()}" class="form-control" disabled>
            </div>
          </div>
        </div>
      </div>
      <div class="col-lg-4">
        ...
        <div class="bg-light p-30 mb-5">
          <img id="imageProduct" src="${requestScope.user.getImage()}"  alt="image" style="width: 100%;height: 330px;">
        </div>
